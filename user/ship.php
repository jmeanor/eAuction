...
<?php
    ...
    elseif($phase == 'item')
	{
		...
		$row = $stmt->fetch(); 
		if($row['failure_notification_date'] == '' || $row['failure_notification_date'] == $null_date_string)
		{
         $item_wdate = date('D, M. j, Y h:i:s A', strtotime($row['date_won']));
         
         $ready_success = true;
         
         if($row['item_received_date'] != $null_date_string || $row['item_received_date'] != '')
         {
         ...
         if($row['item_sent_date'] != $null_date_string || $row['item_sent_date'] != '')
         {
         ...
         if($row['card_charged_date'] != $null_date_string || $row['card_charged_date'] != '')
         {
         ...
         if($row['check_mailed_date'] != $null_date_string || $row['check_mailed_date'] != '')
         {
         ...
         if($row['successful_date'] != $null_date_string || $row['successful_date'] != '')
